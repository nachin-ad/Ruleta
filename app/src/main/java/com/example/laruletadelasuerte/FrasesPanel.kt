package com.example.laruletadelasuerte

class FrasesPanel {
    // Frases para las rondas 1, 2 ,3 y 4
    val frasesPorRonda: Map<Int, List<Pair<String, String>>> = mapOf(

        1 to listOf(
            "   SE BUSCA      UN GATO         CON         SOMBRERO   " to "Titular loco",
            "  LLUVIA DE    HAMBURGUESAS    CAE SOBRE     LA CIUDAD  " to "Titular loco",
            "   CREA UNA    PECERA PARA  SACAR A PASEAR A SUS PECES  " to "Titular loco",
            " ESTE LIQUIDO   ES ESENCIAL    PARA LA         VIDA     " to "Definición de concepto",
            " ESTA PLANTA   GRANDE TIENE   UN TRONCO   LEÑOSO Y HOJAS" to "Definición de concepto"

        ),

        2 to listOf(
            "  LIQUIDO QUE   BEBEMOS Y     PROVIENE DE    UN ANIMAL  " to "Definición de concepto",
            " ESCOGISTE A   LA MAS GUAPA     Y A LA      MENOS BUENA " to "Letra de una canción",
            "HOY PUEDE SER  UN GRAN DIA   PLANTEATELO        ASI     " to "Letra de una canción",
            "   COMO UNA      OLA DE       FUEGO Y DE      CARICIAS  " to "Letra de una canción",
            "  DESCUBREN      FOSFORO     EN SATURNO                 " to "Descubrimiento científico"
        ),

        3 to listOf(
            "  LOS SERES       VIVOS      CAMBIAN CON    EL TIEMPO   " to "Descubrimiento científico",
            "    HALLAN       PLANETAS     SIMILARES     A LA TIERRA " to "Descubrimiento científico",
            " VEO EN TI LO  QUE VAN GOGH  VEIA EN LAS    ESTRELLAS   " to "Panel del amor/desamor",
            "   ERES MI        MEDIA        NARANJA                  " to "Panel del amor/desamor",
            " TU ROSTRO ES LO QUE QUIERO  VER AL FINAL    DEL DIA    " to "Panel del amor/desamor"
        ),

        4 to listOf(
            "  TARTA DE      FRESAS Y       NATA CON     CHOCOLATE   " to "Panel con bote",
            "  VIEIRAS AL    HORNO CON     TARTAR DE      AGUACATE   " to "Panel con bote",
            " PASTA RELLENA  DE DATILES Y  CHAMPIÑONES               " to "Panel con bote"
        )
    )

    // Frases para la ronda final
    val frasesPanelFinal = listOf(
        "    A BUEN        HAMBRE         NO HAY       MAL PAN   " to "Refrán español",
        "   MAS VALE        TARDE      QUE NUNCA                 " to "Refrán español",
        "     A MAL        TIEMPO         BUENA         CARA     " to "Refrán español",
        "  CREA FAMA     Y ECHATE        A DORMIR                " to "Refrán español",
        "               LA ALHAMBRA    Y GRANADA                 " to "Monumento y ciudad",
        " LA CELESTINA  Y FERNANDO      DE ROJAS                 " to "Libro y autor",
        "   DON JUAN     TENORIO       Y JOSE        ZORRILLA    " to "Libro y autor",
        "  BRILLANTE    LUMINOSO Y    RELUCIENTE                 " to "Tres sinónimos",
        "   HERMOSO       BELLO Y      PRECIOSO                  " to "Tres sinónimos",
        "  LA SAGRADA    FAMILIA Y     BARCELONA                 " to "Monumento y ciudad"
    )
}